...
st.set_page_config(layout="wide", page_title="Legacy Master 33.0", page_icon="🏢")
...
with sb.expander("🎲 Simulations & Risk", expanded=False):
...
with sb.expander("🏠 Real Estate Portfolio", expanded=True):
    ...
    for i, p in enumerate(inp["props"]):
        ...
        p["p_year"] = st.number_input(f"Year Purchased {i+1}", 1990, 2026, int(p.get("p_year", 2020)), key=f"py{i}")
        p["rate"] = st.number_input(f"Int Rate % {i+1}", 0.1, 15.0, float(p.get("rate", 0.045)*100), key=f"r_rate{i}") / 100
...
with sb.expander("🎓 Education (Calendar Years)", expanded=True):
    st.caption("Aaron's Tuition Years")
    c1, c2 = st.columns(2)
    inp["k1_s_yr"] = c1.number_input("Start Year", 2026, 2050, int(inp.get("k1_s_yr", 2038)), key="k1s")
    inp["k1_e_yr"] = c2.number_input("End Year", 2026, 2060, int(inp.get("k1_e_yr", 2042)), key="k1e")
    inp["k1_cost"] = st.number_input("Aaron $/yr", value=float(inp["k1_cost"]))
    
    st.caption("Alvin's Tuition Years")
    c3, c4 = st.columns(2)
    inp["k2_s_yr"] = c3.number_input("Start Year", 2026, 2050, int(inp.get("k2_s_yr", 2040)), key="k2s")
    inp["k2_e_yr"] = c4.number_input("End Year", 2026, 2060, int(inp.get("k2_e_yr", 2044)), key="k2e")
    inp["k2_cost"] = st.number_input("Alvin $/yr", value=float(inp["k2_cost"]))

with sb.expander("🛒 Lifestyle & Tax", expanded=False):
...
def run_engine(p_in, sims, monte):
    ...
    for _ in range(sims):
        ...
        for age in range(p_in["ca"], p_in["ea"] + 1):
            ...
            # Education spikes by Calendar Year
            edu_spike = 0
            if p_in["k1_s_yr"] <= yr <= p_in["k1_e_yr"]: edu_spike += p_in["k1_cost"]
            if p_in["k2_s_yr"] <= yr <= p_in["k2_e_yr"]: edu_spike += p_in["k2_cost"]
            
            spend = (p_in["ew"] if (age < p_in["yr"] or age < p_in["hr"]) else p_in["er"]) + edu_spike
...
# --- 4. RHS DASHBOARD ---
st.title("🛡️ Legacy Master v33.0")
...
st.plotly_chart(go.Figure([
    ...
    go.Scatter(x=med_path["Age"], y=p5, fill='tonexty', fillcolor='rgba(239, 68, 68, 0.15)', name="Crash Risk (5%)"),
    go.Scatter(x=med_path["Age"], y=p50, line=dict(color="#10b981", width=4), name="Median Net Worth")
]).update_layout(title="Wealth Probability (Mortgage Amortization Active)", template="plotly_dark"), use_container_width=True)

fig_cf = go.Figure()
for col, color, name in [("Sal", "#34d399", "Income"), ("RE_NOI", "#60a5fa", "Rent (Net P&I)"), ("Tax", "#f87171", "Taxes"), ("Spend", "#fbbf24", "Expenses & Tuition"), ("Draw", "#8b5cf6", "401k Draw")]:
...
st.plotly_chart(fig_eq.update_layout(barmode='stack', title="Asset Composition & Pay-down", template="plotly_dark"), use_container_width=True)
