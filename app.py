...
st.set_page_config(layout="wide", page_title="Legacy Master 30.0", page_icon="🏠")
...
with sb.expander("🏠 Real Estate Portfolio", expanded=True):
    ...
    for i, p in enumerate(inp["props"]):
        ...
        c1, c2 = st.columns(2)
        p["liq_active"] = c1.checkbox("Liquidate?", value=p.get("liq_active", True), key=f"la{i}")
...
# --- 4. DASHBOARD (RHS) ---
st.title("🛡️ Legacy Master v30.0")
...
st.plotly_chart(go.Figure([
    ...
    go.Scatter(x=med_path["Age"], y=p5, fill='tonexty', fillcolor='rgba(239, 68, 68, 0.1)', name="5th Percentile (Crash Risk)"),
    go.Scatter(x=med_path["Age"], y=p50, line=dict(color="#10b981", width=4), name="Median NW")
]).update_layout(title="Wealth Probability (Market Volatility Engine Active)", template="plotly_dark"), use_container_width=True)
...
st.plotly_chart(fig_cf.update_layout(barmode='relative', title="Granular Cash Flow & Education Spikes", template="plotly_dark"), use_container_width=True)
...
st.plotly_chart(fig_eq.update_layout(barmode='stack', title="Asset Composition & Equity Pay-down", template="plotly_dark"), use_container_width=True)
