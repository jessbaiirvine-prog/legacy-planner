...
st.set_page_config(layout="wide", page_title="Legacy Master 26.0", page_icon="🏠")
...
if "inputs" not in st.session_state:
    st.session_state.inputs = {
        ...
        "liq_age": 55, "props": [
            {"v": 1700000.0, "b": 1000000.0, "l": 800000.0, "p_year": 2020, "term": 30, "rate": 0.045, "r": 6500.0, "a": 0.03, "liq": True}
        ],
        "k1_start": 18, "k1_end": 22, "k1_cost": 45000.0, "k2_start": 18, "k2_end": 22, "k2_cost": 45000.0,
...
with sb.expander("🏠 Real Estate Engine", expanded=True):
    inp["liq_age"] = st.number_input("Global Liquidation Age", 45, 85, int(inp["liq_age"]))
    n_p = st.number_input("Property Count", 1, 10, len(inp["props"]))
    while len(inp["props"]) < n_p: 
        inp["props"].append({"v": 1000000.0, "b": 800000.0, "l": 500000.0, "p_year": 2024, "term": 30, "rate": 0.065, "r": 4500.0, "a": 0.03, "liq": False})
    ...
    for i, p in enumerate(inp["props"]):
        ...
        p["l"] = st.number_input(f"Orig. Loan Amount {i+1}", value=float(p["l"]), key=f"l{i}")
        p["p_year"] = st.number_input(f"Year Purchased {i+1}", 1990, 2026, int(p["p_year"]), key=f"py{i}")
        p["term"] = st.number_input(f"Mortgage Term (Yrs) {i+1}", 5, 40, int(p["term"]), key=f"t{i}")
        p["rate"] = st.number_input(f"Int. Rate % {i+1}", 0.1, 15.0, float(p["rate"]*100), key=f"r_rate{i}") / 100
        p["r"] = st.number_input(f"Monthly Rent {i+1}", value=float(p["r"]), key=f"rent{i}")
        p["a"] = st.number_input(f"Appreciation % {i+1}", 0.0, 10.0, float(p["a"]*100), key=f"appr{i}") / 100
        p["liq"] = st.checkbox(f"Sell at age {inp['liq_age']}?", value=p.get("liq", False), key=f"liq{i}")
        ...
        for age in range(p_in["ca"], p_in["ea"] + 1):
            ...
            for pr in props:
                if pr["v"] > 0:
                    ...
                    cv = pr["v"] * ((1 + (env["R"] if monte else pr["a"]))**h)
                    
                    # Liquidation Event
                    if age == p_in["liq_age"] and pr.get("liq"):
                        cc += (cv - rem_bal - (max(0, cv - pr.get("b", 1000000)) * p_in["cap_gains"]))
                        pr["v"] = 0
                    else:
                        re_val += cv
                        re_eq += (cv - rem_bal)
            ...
            aaron_age, alvin_age = age - 32, age - 30
            edu = (p_in["k1_cost"] if p_in["k1_start"] <= aaron_age <= p_in["k1_end"] else 0) + \
                  (p_in["k2_cost"] if p_in["k2_start"] <= alvin_age <= p_in["k2_end"] else 0)
...
# --- 4. DASHBOARD ---
st.title("🛡️ Legacy Master v26.0")
...
]).update_layout(title="Wealth Probability & Amortization Pay-down", template="plotly_dark")
...
st.plotly_chart(fig_cf.update_layout(barmode='relative', title="Cash Flow: Mortgage Impact Shown in 'Rent'", template="plotly_dark"), use_container_width=True)

# Asset composition
fig_eq = go.Figure()
fig_eq.add_trace(go.Bar(x=med_path["Age"], y=med_path["RE_Val"], name="RE Market Value", marker_color="#1e40af"))
...
st.plotly_chart(fig_eq.update_layout(barmode='stack', title="Asset Composition: See Equity vs. Debt Pay-down", template="plotly_dark"), use_container_width=True)
