import streamlit as st
import pandas as pd
import plotly.graph_objects as go
import numpy as np
import json

st.set_page_config(layout="wide", page_title="Legacy Master 36.0", page_icon="🛡️")

# --- 1. INITIALIZATION & SAFETY DEFAULTS ---
DEFAULT_PROP = {"v": 1700000.0, "b": 1000000.0, "l": 800000.0, "p_year": 2020, "term": 30, "rate": 0.045, "r": 6500.0, "a": 0.03, "liq_age": 55, "liq_active": True}
DEFAULTS = {
    "v_cash": 200000.0, "v_brokerage": 500000.0, "v_401k": 1200000.0,
    "tax_work": 0.30, "tax_ret": 0.20, "cap_gains": 0.20, "target_roi": 0.07, "volatility": 0.15,
    "p_tax": 0.012, "p_maint": 0.01, "p_mgmt": 0.08,
    "props": [DEFAULT_PROP.copy()],
    "k1_s_yr": 2038, "k1_e_yr": 2042, "k1_cost": 50000.0, 
    "k2_s_yr": 2040, "k2_e_yr": 2044, "k2_cost": 50000.0,
    "ca": 42, "ea": 95, "hp": 145000.0, "hr": 58, "yp": 110000.0, "yr": 55, 
    "ew": 150000.0, "er": 120000.0, "ss": 85000.0,
    "n_sims": 500, "use_monte": True
}

if "inputs" not in st.session_state:
    st.session_state.inputs = DEFAULTS.copy()

inp = st.session_state.inputs
sb = st.sidebar

# --- 2. SIDEBAR (LHS) ---
sb.title("💾 Scenario Setup")
uploaded_file = sb.file_uploader("Upload Scenario", type="json")
if uploaded_file:
    inp.update(json.load(uploaded_file))
    st.rerun()

# CRITICAL: Define Simulation Parameters Early to Prevent NameError
with sb.expander("🎲 Simulations & Risk", expanded=True):
    use_monte = st.toggle("Monte Carlo Mode", value=inp.get("use_monte", True))
    n_sims = st.slider("Number of Simulations", 10, 2000, int(inp.get("n_sims", 500)))
    inp["target_roi"] = st.slider("Expected ROI %", 0.0, 15.0, float(inp.get("target_roi", 0.07)*100))/100
    inp["volatility"] = st.slider("Volatility %", 5, 30, int(inp.get("volatility", 0.15)*100))/100

with sb.expander("📈 Liquid Investments", expanded=True):
    inp["v_401k"] = st.number_input("401k Balance", value=float(inp["v_401k"]))
    inp["v_brokerage"] = st.number_input("Brokerage Balance", value=float(inp["v_brokerage"]))
    inp["v_cash"] = st.number_input("Cash / Savings", value=float(inp["v_cash"]))

with sb.expander("🏠 Real Estate Assets", expanded=True):
    n_p = st.number_input("Properties", 1, 10, len(inp["props"]))
    while len(inp["props"]) < n_p: inp["props"].append(DEFAULT_PROP.copy())
    inp["props"] = inp["props"][:n_p]
    for i, p in enumerate(inp["props"]):
        st.markdown(f"**Asset {i+1}**")
        p["v"] = st.number_input(f"Val {i+1}", value=float(p["v"]), key=f"v{i}")
        p["l"] = st.number_input(f"Loan {i+1}", value=float(p["l"]), key=f"l{i}")
        c1, c2 = st.columns(2)
        p["liq_active"] = c1.checkbox("Sell?", value=p.get("liq_active", True), key=f"la{i}")
        p["liq_age"] = c2.number_input("At Age", 45, 95, int(p.get("liq_age", 55)), key=f"lage{i}")

with sb.expander("💵 Income & Education", expanded=False):
    inp["hp"], inp["hr"] = st.number_input("Husband Salary", value=float(inp["hp"])), st.number_input("Husband Retire", value=int(inp["hr"]))
    inp["yp"], inp["yr"] = st.number_input("Your Salary", value=float(inp["yp"])), st.number_input("Your Retire", value=int(inp["yr"]))
    st.divider()
    inp["k1_s_yr"], inp["k1_e_yr"] = st.number_input("Aaron Start Yr", 2026, 2050, int(inp["k1_s_yr"])), st.number_input("Aaron End Yr", 2026, 2060, int(inp["k1_e_yr"]))
    inp["k2_s_yr"], inp["k2_e_yr"] = st.number_input("Alvin Start Yr", 2026, 2050, int(inp["k2_s_yr"])), st.number_input("Alvin End Yr", 2026, 2060, int(inp["k2_e_yr"]))

# --- 3. MATH ENGINE ---
@st.cache_data
def run_engine(p_in, sims, monte):
    all_runs = []
    for _ in range(sims):
        cash, brok, ret = p_in["v_cash"], p_in["v_brokerage"], p_in["v_401k"]
        props = [pr.copy() for pr in p_in["props"]]
        path = []
        for age in range(p_in["ca"], p_in["ea"] + 1):
            h, yr = age - p_in["ca"], 2026 + (age - p_in["ca"])
            m_ret = np.random.normal(p_in["target_roi"], p_in["volatility"]) if monte else p_in["target_roi"]
            
            re_val, re_eq, re_cf = 0, 0, 0
            for pr in props:
                if pr["v"] > 0:
                    mi, mt = pr.get("rate", 0.045)/12, pr.get("term", 30)*12
                    pmt_mo = pr["l"] * (mi * (1 + mi)**mt) / ((1 + mi)**mt - 1) if pr["l"] > 0 else 0
                    mos_passed = (yr - pr.get("p_year", 2020)) * 12
                    rem_bal = pr["l"] * ((1 + mi)**mt - (1 + mi)**max(0, mos_passed)) / ((1 + mi)**mt - 1) if mos_passed < mt else 0
                    cv = pr["v"] * ((1 + pr.get("a", 0.03))**h)
                    if age == pr.get("liq_age", 55) and pr.get("liq_active"):
                        cash += (cv - rem_bal - (max(0, cv - pr.get("b", 1000000)) * p_in["cap_gains"])); pr["v"] = 0
                    else:
                        re_val += cv; re_eq += (cv - rem_bal)
                        re_cf += (pr["r"]*12) - (cv*(p_in["p_tax"]+p_in["p_maint"])) - (pr["r"]*12*p_in["p_mgmt"]) - (pmt_mo * 12)

            sal = (p_in["hp"] if age < p_in["hr"] else 0) + (p_in["yp"] if age < p_in["yr"] else 0)
            tax_inc = sal + max(0, re_cf) + (p_in["ss"] if age >= 67 else 0)
            tax = tax_inc * (p_in["tax_work"] if sal > 0 else p_in["tax_ret"])
            edu = (p_in["k1_cost"] if p_in["k1_s_yr"] <= yr <= p_in["k1_e_yr"] else 0) + (p_in["k2_cost"] if p_in["k2_s_yr"] <= yr <= p_in["k2_e_yr"] else 0)
            spend = (p_in["ew"] if (age < p_in["yr"] or age < p_in["hr"]) else p_in["er"]) + edu
            net = (tax_inc - tax) - spend
            
            draw_401k = 0
            if net < 0:
                gap = abs(net)
                f_cash = min(cash, gap); cash -= f_cash; gap -= f_cash
                if gap > 0: f_brok = min(brok, gap); brok -= f_brok; gap -= f_brok
                if gap > 0: d = min(ret, gap / (1 - p_in["tax_ret"])); ret -= d; draw_401k = d
            else: cash += net

            brok *= (1 + m_ret); ret *= (1 + m_ret); cash *= 1.02
            path.append({"Age": age, "NW": cash+brok+ret+re_eq, "Sal": sal, "RE_NOI": re_cf, "Draw": draw_401k, "Liq": cash+brok+ret})
        all_runs.append(path)
    return all_runs

# --- 4. EXECUTION & DISPLAY ---
results = run_engine(inp, n_sims, use_monte)
nw_mat = np.array([[y["NW"] for y in path] for path in results])
p5, p50, p95 = np.percentile(nw_mat, [5, 50, 95], axis=0)
med_path = pd.DataFrame(results[len(results)//2])

st.title("🛡️ Legacy Master v36.0")
m1, m2, m3, m4 = st.columns(4)
m1.metric("Median Estate", f"${p50[-1]:,.0f}"); m2.metric("Success Rate", f"{(nw_mat[:,-1]>0).mean()*100:.1f}%"); m3.metric("Worst Case (5%)", f"${p5[-1]:,.0f}"); m4.metric("Simulations", n_sims)

st.plotly_chart(go.Figure([
    go.Scatter(x=med_path["Age"], y=p95, line=dict(width=0), showlegend=False),
    go.Scatter(x=med_path["Age"], y=p5, fill='tonexty', fillcolor='rgba(239, 68, 68, 0.15)', name="Downside Risk"),
    go.Scatter(x=med_path["Age"], y=p50, line=dict(color="#10b981", width=4), name="Median Net Worth")
]).update_layout(title="Wealth Probability (Stability Fix Applied)", template="plotly_dark"), use_container_width=True)

# Audit Summary
st.divider()
st.header("📋 Post-Simulation Audit")
crisis = med_path[med_path["Draw"] > 0]
c1, c2 = st.columns(2)
with c1:
    st.subheader("🚩 Crisis Detection")
    if not crisis.empty: st.error(f"Drawdown Years: Ages {crisis['Age'].min()} to {crisis['Age'].max()}")
    else: st.success("Liquidity is stable throughout simulation.")
with c2:
    st.subheader("💡 Strategy")
    st.write("Current simulation shows high sensitivity to Market Volatility during education years. Consider increasing cash reserves to 24 months of spend before retirement.")

st.download_button("📥 Save JSON", data=json.dumps(inp), file_name="scenario.json")
